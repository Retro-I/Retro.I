...
class SettingsSyncHelper:
    def sync_values(self, source, target, template_source) -> dict | list:
        """
        Recursively sync source → target without overwriting target values.
    ...
    def repair_settings_file(self, filename: str):
        default_settings_path = f"{c.default_settings_path()}/settings/{filename}"
        ...
        if os.path.exists(f"{c.default_settings_path()}/settings/templates/template_{filename}"):
            with open(settings_path, "r") as f:
        ...
        with open(settings_path, "w") as f:
            ...
            print(f"Updated settings file: {filename}")
